...
is
   ...
   the_Observer :         Observer.deferred.view := Observer.deferred.forge.new_Observer ("demo.Observer");

   the_Response : aliased key_Response           := (Response.item with
                                                     key_to_message_Map => ('a'    => to_unbounded_String ("Howdy"),
                                                                            others => to_unbounded_String ("Nothing to report.")));
...
begin
   ...
   for Each in 1 .. 5
   loop                                                                            -- Our main loop.
      the_Observer.respond;                                                        -- Responses to any queued events occur here.
