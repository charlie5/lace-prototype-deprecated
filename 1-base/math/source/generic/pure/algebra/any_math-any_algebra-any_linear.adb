...
is
   ...
   is
      use Vectors;
      inv_Norm : constant Real := 1.0 / abs (Self);
   begin
      for Each in Self'Range
      loop
   ...
   begin
      ...
      for Each in the_Min'Range
      loop
         the_Min (Each) := Real'Min (Left  (Each),
                                     Right (Each));
      end loop;
   ...
   begin
      ...
      for Each in the_Max'Range
      loop
         the_Max (Each) := Real'Max (Left  (Each),
                                     Right (Each));
      end loop;
   ...
   is
      the_Image : String (1 .. 1024 * 1024);         -- Handles one megabyte image, excess is truncated.
   ...
   begin
      for each_Row in the_sub_Matrix'Range (1)
      loop
         for each_Col in the_sub_Matrix'Range (2)
         loop
   ...
   is
      ...
      unit       : constant Real :=      sqx + sqy + sqz + sqw;      -- If normalised is one, otherwise is correction factor.
