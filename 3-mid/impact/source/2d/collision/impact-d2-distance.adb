...
package body impact.d2.Distance
is
   use Interfaces;
   ...
   procedure set (Self : in out b2DistanceProxy;   shape : in impact.d2.Shape.view;
                                                   index : in int32)
   is
      use impact.d2.Shape;
   begin
      case shape.getKind is
         when e_circle =>
            declare
               circle : impact.d2.Shape.circle.view := impact.d2.shape.circle.view (shape);
            begin
         ...
         when e_polygon =>
            declare
               polygon : impact.d2.Shape.polygon.view := impact.d2.Shape.polygon.view (shape);
            begin
               Self.m_vertices := polygon.m_vertices (1)'Access;
               Self.m_count    := polygon.m_vertexCount;
...
--              break;
   ...
   is
      use b2Vec2_Pointers;
      the_Array : b2Vec2_array renames Value (Self.m_vertices, C.ptrdiff_t (Self.m_count));
   begin
--        put_Line ("ID: " & int32'Image (index));
      pragma Assert (1 <= index and then index <= Self.m_count);

      return the_Array (uint32 (Index) - 1);
   end getVertex;
