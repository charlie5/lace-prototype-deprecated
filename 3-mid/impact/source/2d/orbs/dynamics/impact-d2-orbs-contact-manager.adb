...
package body impact.d2.orbs.contact.Manager
is
   use type int32;
   ...
   procedure Destroy (Self : in out b2ContactManager;   c : in out impact.d2.orbs.Contact.view)
   is
      use type Contact.view;

      fixtureA : constant access Fixture.item     := c.GetFixtureA;
      fixtureB : constant access Fixture.item     := c.GetFixtureB;

      bodyA    : constant access Solid.item'Class := fixtureA.GetBody;
      bodyB    : constant access Solid.item'Class := fixtureB.GetBody;
   ...
   begin
      ...
      while c /= null loop
         declare
            fixtureA : constant access Fixture.item'Class  := c.GetFixtureA;
            fixtureB : constant access Fixture.item'Class  := c.GetFixtureB;
   ...
   is
      ...
      fixtureA : access Fixture.item := Fixture.view (userDataA);
      fixtureB : access Fixture.item := Fixture.view (userDataB);
   ...
   begin
      ...
      while edge /= null loop
         if edge.other = bodyA then
            declare
               fA : constant access Fixture.item := edge.contact.GetFixtureA;
               fB : constant access Fixture.item := edge.contact.GetFixtureB;
...
end impact.d2.orbs.contact.Manager;
