...
extern "C"
{
  ...
  void
  b3d_Joint_user_Data_is   (Joint*   Self,   void*   Now)
  {
    //  btTypedConstraint*   the_Joint = to_bullet_Joint (Self);

    // tbd: the_Joint->setUserPointer (Now);
  }
  ...
  bool
  b3d_Joint_Extent (Joint*   Self,   int    DoF)
  {
    btTypedConstraint*    c_Self = to_bullet_Joint (Self);
  ...
  {
    // tbd: this does not apply to all types ... have to check type and then use switch to convert to correct bullet joint pointer.
  ...
  void
  b3d_Joint_Frame_A_is     (Joint*   Self,   Matrix_4x4*   Now)
  {
    1/0;
  }
  ...
  void
  b3d_Joint_Frame_B_is     (Joint*   Self,   Matrix_4x4*   Now)
  {
    1/0;
  }
  ...
  bool
  b3d_Joint_is_Limited     (Joint*   Self,   int           DoF)
  {
    return 1/0;
  }
  ...
  void
  b3d_Joint_Velocity_is    (Joint*   Self,   int           DoF,
			                     Real          Real)
  {
    1/0;
  }
  ...
  void
  b3d_Joint_hinge_Limits_are (Joint*   Self,   Real           Lower,
			                       Real           Upper,
			                       Real           Softeness,
			                       Real           bias_Factor,
			                       Real           relaxation_Factor)
  {
    ...
    c_Self->setLimit (Lower, Upper,
		      Softeness, bias_Factor, relaxation_Factor);
  }
  ...
  void
  b3d_Joint_6DoF_lower_Limit_is (Joint*   Self,   int          DoF,
				                  Real         Now)
  {
  ...
  void
  b3d_Joint_6DoF_upper_Limit_is (Joint*   Self,   int           DoF,
                                                  Real          Now)
  {
    btGeneric6DofConstraint*    c_Self    = (btGeneric6DofConstraint*) to_bullet_Joint (Self);
    btRotationalLimitMotor *    the_Motor = c_Self->getRotationalLimitMotor (DoF - 4);
  ...
  Real
  b3d_Joint_6DoF_lower_Limit    (Joint*   Self,   int           DoF)
  {
    btGeneric6DofConstraint*    c_Self = (btGeneric6DofConstraint*) to_bullet_Joint (Self);
  ...
  Real
  b3d_Joint_6DoF_upper_Limit    (Joint*   Self,   int           DoF)
  {
    btGeneric6DofConstraint*    c_Self = (btGeneric6DofConstraint*) to_bullet_Joint (Self);
