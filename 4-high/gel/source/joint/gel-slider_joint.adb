...
is
   ...
   is
      ...
      pivot_in_A : constant Math.Vector_3   := pivot_Anchor - Sprite_A.Site;
      pivot_in_B : constant Math.Vector_3   := pivot_Anchor - Sprite_B.Site;

      Frame_A    : constant Math.Matrix_4x4 := to_transform_Matrix (pivot_Axis, pivot_in_A);
      Frame_B    : constant Math.Matrix_4x4 := to_transform_Matrix (pivot_Axis, pivot_in_B);
   begin
   ...
   overriding
   function Frame_A (Self : in Item) return Math.Matrix_4x4 is
   begin
   ...
   overriding
   function Frame_B (Self : in Item) return Math.Matrix_4x4 is
   begin
   ...
   overriding
   procedure Frame_A_is (Self : in out Item; Now : in Math.Matrix_4x4) is
   begin
   ...
   overriding
   procedure Frame_B_is (Self : in out Item; Now : in Math.Matrix_4x4) is
   begin
   ...
   overriding
   function Extent
     (Self : in     Item;   for_Degree : in Joint.Degree_of_freedom) return math.Real
   is
   begin
      if for_Degree in Sway .. Surge
      then
   ...
   overriding
   procedure Velocity_is
     (Self : in     Item;   for_Degree : in Joint.Degree_of_freedom;
                            Now        : in Math.Real)
   is
   begin
